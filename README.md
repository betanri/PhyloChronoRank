...
`PhyloChronoRank` is a post-fit evaluation framework for researchers who already have a set of competing chronograms and need to decide which one is the most biologically defensible.
...
The framework keeps three layers conceptually separate:

- `clock fitting`: which model is preferred by fit statistics
- `lambda tuning`: which smoothing strength is preferred within a fitted model search
- `post-fit evaluation`: which finished dated tree behaves best biologically

`PhyloChronoRank` is about that third layer. It uses three metric families. These are implementation-level diagnostics rather than named published indices; the citations below support the underlying ideas each family is trying to capture.
...
- `RelTime` is the overall post-fit winner in this comparison
...
Figure B uses the same family-balanced rule as the table. Even though three pulse panels are shown, they do not count as three separate thirds. They are averaged into one pulse-family contribution, and that pulse family contributes one-third of the overall rank. Here the gap panel is `mean relative gap`.
...
- A plausible future extension is an `uncertainty width` layer for methods that return node-age confidence or credible intervals. That metric would speak to precision, not accuracy, so it should be reported as an optional fourth family rather than folded blindly into the current overall rank.
