...
The pipeline also reports upstream `chronos` fit statistics. Those are useful, but they should not be treated as the final decision rule by themselves: our simulations suggest that `PHIIC` does not always recover the true generating clock model reliably. Across both simulations and the bundled empirical examples, `chronos_clock` and `chronos_discrete` are often the strongest `chronos` performers, whereas `chronos_correlated` and `chronos_relaxed` tend to perform worse, including on the downstream post-fit biological metrics. One notable pattern is that `chronos_clock` is often the top-performing tree among all six candidates, even under fairly high heterotachy. That broad pattern is consistent with [Paradis 2013](https://doi.org/10.1016/j.ympev.2013.02.008), which reported good strict-clock estimates even under heterogeneous substitution rates, and with [Paradis et al. 2023](https://doi.org/10.1016/j.ympev.2022.107652), which suggested that the strict clock can remain competitive in part because it simplifies optimization by estimating a single rate rather than a branch-specific rate process.
