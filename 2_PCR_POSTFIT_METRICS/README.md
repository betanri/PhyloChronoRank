...
- `tempo_redistribution` — How much the dating method reshuffles the timing of non-burst nodes relative to the phylogram. Measured as the earth mover's distance between non-burst node-depth distributions in the phylogram vs. chronogram. Less redistribution is better: it means the dating method preserved the phylogram's temporal backbone rather than rearranging it. **Caveat:** Bayesian dating methods that use a constant-rate birth-death tree prior (e.g., MCMCTree, BEAST) may score artificially well (low EMD) on this metric. The BD prior smooths backbone node depths toward the prior's expected distribution, which can reduce the EMD independently of how well the method estimated divergence times ([Mello & Kumar 2021](https://doi.org/10.1111/1755-0998.13249)). A low `tempo_redistribution` from a BD-prior method should not be taken at face value.
